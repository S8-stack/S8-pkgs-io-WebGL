...
export function transformVector4d(m, v, w, vOffset=0, wOffset=0) {
	let x = v[vOffset+0], y = v[vOffset+1], z = v[vOffset+2], w = v[vOffset+3];
	w[wOffset+0] = m[00] * x + m[04] * y + m[08] * z + m[12] * w;
	w[wOffset+1] = m[01] * x + m[05] * y + m[09] * z + m[13] * w;
	w[wOffset+2] = m[02] * x + m[06] * y + m[10] * z + m[14] * w;
	w[wOffset+3] = m[03] * x + m[07] * y + m[11] * z + m[15] * w;
};
 ...
 */
export function transformVector3d(m, v, w, vOffset=0, wOffset=0) {
	let x = v[vOffset+0], y = v[vOffset+1], z = v[vOffset+2];
	w[wOffset+0] = m[00] * x + m[04] * y + m[08] * z;
	w[wOffset+1] = m[01] * x + m[05] * y + m[09] * z;
	w[wOffset+2] = m[02] * x + m[06] * y + m[10] * z;
};
 ...
 * @param {number} wOffset 
 */
 export function transformPoint3d(m, v, w, vOffset=0, wOffset=0) {
	let x = v[vOffset+0], y = v[vOffset+1], z = v[vOffset+2];
	w[wOffset+0] = m[00] * x + m[04] * y + m[08] * z + m[12];
	w[wOffset+1] = m[01] * x + m[05] * y + m[09] * z + m[13];
	w[wOffset+2] = m[02] * x + m[06] * y + m[10] * z + m[14];
};
